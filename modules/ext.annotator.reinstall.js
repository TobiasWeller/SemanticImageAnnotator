(function() {
    if ($('#reinstall-btn').length) {
        ...
        $('#reinstall-btn').click(function() {
            swal({
                title: mw.msg('sia-reinstall-header'),
                text: mw.msg('sia-request-reinstall'),
                type: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: mw.msg('sia-confirm-reinstall')
            }).then(function() {
                appendLoadingSpinner();
                //Form neu Installieren
                api.createPage('Form:ImageAnnotation', getFormText(), function() {
                    window.location.reload(false);
                });
                //Template neu Installieren
                api.createPage('Template:ImageAnnotation', getTemplateText(), function() {
                    window.location.reload(false);
                });
                //ImageAnnotation Category
                api.createPage('Category:ImageAnnotation', 'This is the ImageAnnotation category used by Semantic Image Annotator.\n\n[[Category:Annotation]]', function() {
                    window.location.reload(false);
                });
                //Annotation Category
...
function replaceSubobject(pagename) {
    api.getPageContent(pagename, function(content) {
        var tmp = content;
        content = content.replace(/({{#subobject:SemanticImageAnnotator)(.|\n)*?(}})/i, "");
        if (content == tmp) {
...
function getTemplateText() {
    var text = '<noinclude>' + '\n';
    text += 'This is the "ImageAnnotation" template.' + '\n';
