...
( function () {
	...
	$('#p-views>ul').append('<li id="ca-annotate"><span><a href="#" title="'+mw.msg('sia-annotate-button-desc')+'" accesskey="a">'+mw.msg('sia-annotate-button-text')+'</a><i class="fa fa-check" aria-hidden="true"></i></span></li>');
	...
	$('#ca-annotate').click(function() {
		...
		if(loaded){
			mw.loader.using( 'ext.imageannotator.module' ).then( function () {
				...
				mw.notify( mw.message('sia-annotate-welcome-message') );
			} );
		}else{
			//$('#content').annotator('destroy');
            $( '#ca-annotate' ).removeClass( 'selected' );
			mw.notify( mw.message('sia-annotate-godbye-message') );
			location.reload();
		}
	});
}() );
